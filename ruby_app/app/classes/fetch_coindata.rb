...
class CoinData
  API_KEY = '38f1ded0d6e9d90c81b274ccccdabe9bf320ab6f'
  ...
  def task_4_output(compare_first: '', compare_second: '')
    call_api(payload: @payload)
    tick1 = get_ticker(compare_first)['price'].to_f
    tick2 = get_ticker(compare_second)['price'].to_f
    (tick1 / tick2).round(2)
  end
  ...
  def call_api(payload: [], currency: 'USD')
    uri = URI("https://api.nomics.com/v1/currencies/ticker?key=#{API_KEY}" \
      "&ids=#{payload.join(',')}&interval=1d&convert=#{currency}&per-page=100&page=1")
